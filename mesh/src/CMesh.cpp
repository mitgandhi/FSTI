...
void CMesh::ReadBoundaries(void)
{
	...
	if(body == 1 && myinput.data.thermal.piston.mixed_bc.size()){//piston
		for(int i=0; i<myinput.data.thermal.piston.mixed_bc.size(); i++){
			// Check if h is zero, use default value 2500 if true
			double h = myinput.data.thermal.piston.mixed_bc[i].h;
			h = (h == 0) ? 2500 : h;
			hbi.push_back(h);
        
			// Check if Tinf is zero, use default value 50 if true
			double Tinf = myinput.data.thermal.piston.mixed_bc[i].Tinf;
			Tinf = (Tinf == 0) ? 50 : Tinf;
			phibinfi.push_back(Tinf);
		};
	}
	else if(body == 0 && myinput.data.thermal.block.mixed_bc.size()){//bushing
		for(int i=0; i<myinput.data.thermal.block.mixed_bc.size(); i++){
			// Check if h is zero, use default value 2500 if true
			double h = myinput.data.thermal.block.mixed_bc[i].h;
			h = (h == 0) ? 2500 : h;
			hbi.push_back(h);
        
			// Check if Tinf is zero, use default value 50 if true
			double Tinf = myinput.data.thermal.block.mixed_bc[i].Tinf;
			Tinf = (Tinf == 0) ? 50 : Tinf;
			phibinfi.push_back(Tinf);
		};
};
