...
double HLP32_oil::get_K(double _p, double _T)
{
	...
	double RhoT = rs*(1-als*T); 
	double RhoTp = RhoT/(1 - a1*log((a2 + a3*T + p*1.0e-5)/(a2 + a3*T)));
	return 1.0e5*(RhoT*(a2 + a3*T + p*1.0e-5)/(a1*RhoTp)); //[Pa]
}
// ------------------------------------------------------------------------- //
...
double Parker_skydrol_oil::get_rho(double p, double T)
{
	...
	//Specified temperature and pressure
		crho.T_degF = T*9/5+32;
		crho.p_psi = p*14.504*1e-5;
	...
	// Density
        ...
        Beta_at = get_K(p,T) * 1e-5 * 14.504;									 //Adiabatic tangent bulk modulus at specified conditions
