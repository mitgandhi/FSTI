...
void CPistonGap::PistonReynoldsCalcCoefficientsMG(void)
{
	...
	while(l<nL)
	{
		...
		double mun,mus,mue,muw,hKn,hKs,hKe,hKw,hn,hs,he,hw,defsqueezeK,defsqueezeB;
		...
		for(int i=0;i<N*M;i++)
		{
			...
			//North
			levels[l].an(i) = pow(hn,3.0) * dx / ( dy * 6.0 * mun );
			if(i%M==(M-1) && i>0){
				levels[l].an(i) *= 2.0;	};
			//South
			levels[l].as(i) = pow(hs,3.0) * dx / ( dy * 6.0 * mus );
			if(i%M==0){
				levels[l].as(i) *= 2.0;	};
			//East
			levels[l].ae(i) = pow(he,3.0) * dy / ( dx * 6.0 * mue );
			//West
			levels[l].aw(i) = pow(hw,3.0) * dy / ( dx * 6.0 * muw );
			//Point
			...
			if(l==0)
			{
				...
				//Source
				levels[l].b(i) = ( - speedK * omega * rK * (he - hw) / dx - vK * (hn - hs) / dy 
