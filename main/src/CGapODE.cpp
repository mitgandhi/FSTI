...
#include <boost_1_53_0\boost\math\special_functions\fpclassify.hpp>
#pragma once
...
{	
	...
	//Final value for x
	xend = abs(myinput.data.lubrication_module.n_lubrication_revolutions) / (myinput.data.operating_conditions.speed/(2*PI));
...
// converts the infinite values to the Nan values 
std::string checkNanInfinity(double value){
...
int CGapODE::ODESolutionOutput(void)
{
	...
	if ( xold==0.0 || x>(xd+tol) ) 
	{
		...
		if(myinput.data.options_piston.general.inclined_piston){
			...
			fout << scientific << checkNanInfinity(myGapResult.time)												//1
				<< "\t" << checkNanInfinity(myinput.data.operating_conditions.speed * myGapResult.time / (2*PI))	//2
				<< "\t" << checkNanInfinity(myGapResult.phi)														//3
				<< "\t" << checkNanInfinity(myGapResult.gappositions[1])											//4
				<< "\t" << checkNanInfinity(myGapResult.gappositions[2])											//5
				<< "\t" << checkNanInfinity(myGapResult.gappositions[3])											//6
				<< "\t" << checkNanInfinity(myGapResult.gappositions[4])											//7
				<< "\t" << checkNanInfinity(myGapResult.gapvelocities[0])											//8
				<< "\t" << checkNanInfinity(myGapResult.gapvelocities[1])											//9
				<< "\t" << checkNanInfinity(myGapResult.gapvelocities[2])											//10
				<< "\t" << checkNanInfinity(myGapResult.gapvelocities[3])											//11
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.lvar)					//12
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.lout)					//13
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.pos_A)				//14
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.pos_B)				//15
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.zRK)					//16
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.sK)					//17
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.lA)					//18
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.lB)					//19
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.z_A)					//20
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.geometrypistongap.z_B)					//21
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.operatingpistongap.vK)					//22
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.operatingpistongap.QSK)					//23
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.operatingpistongap.QSK_p)				//24
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.operatingpistongap.QSK_c)				//25
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MTK)					//26
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MTKtan)					//27
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKy))				//28
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKy_p))			//29
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKy_c))			//30
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKx))			//31
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKx_p))			//32
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKx_c))			//33
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FTG)					//34
				<< "\t" << checkNanInfinity(myGapResult.pDC)														//35
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTby))			//36
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTby_p))			//37
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTby_c))			//38
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTbx))			//39
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTbx_p))			//40
				<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTbx_c))			//41
				<< "\t" << checkNanInfinity(myGapResult.pHP)														//42
				<< "\t" << checkNanInfinity(myGapResult.PLP)														//43
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_external[0])			//44
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_external[1])			//45
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_external[2])			//46
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_external[3])			//47
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_fluid[0])				//48
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_fluid[1])				//49
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_fluid[2])				//50
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_fluid[3])				//51
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_contact[0])			//52
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_contact[1])			//53
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_contact[2])			//54
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.F_contact[3])			//55
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.dF[0])					//56
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.dF[1])					//57
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.dF[2])					//58
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.dF[3])					//59
				<< "\t" << checkNanInfinity(QSKTot)																//60
				<< "\t" << checkNanInfinity(MSKTot)																//61
				<< "\t" << checkNanInfinity(MFT)																	//62
				<< "\t" << checkNanInfinity(PhiD_mech_tot)														//63
				<< "\t" << checkNanInfinity(PhiD_vol_tot)															//64
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.PhiD_mech)								//65
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.PhiD_vol)		                        //66
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FSKx)                   //67
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FSKy)                   //68
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FAKz)                   //69
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FKx)                    //70
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FKy)                    //71
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FK)                     //72
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FaK)                    //73
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FDK)                    //74
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.Fwk)                    //75
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FAKy)                   //76
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.Fwky)                   //77
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.Fwkz)                   //78
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FAK_inclined)           //79
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MKx)                    //80
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MKy)                    //81
				<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MK)                     //82
				<< "\n";
			fout.close();
			fout.clear();
			...
			if(coupled_caspar_simulation){
				...
				fout << scientific << checkNanInfinity(myGapResult.time)											//1
					<< "\t" << checkNanInfinity(myGapResult.phi)													//2
					<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKy))			//3
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FSKx)               //4
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FSKy)               //5
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FAKz)               //6
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FKx)                //7
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FKy)                //8
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FK)                 //9
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FaK)                //10
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FDK)                //11
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.Fwk)                //12
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FAKy)               //13
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.Fwky)               //14
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.Fwkz)               //15
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.FAK_inclined)       //16
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MKx)                //17
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MKy)                //18
					<< "\t" << checkNanInfinity(myNewtonIteration.myPistonGap.forcespistongap.MK)                 //19
					<< "\n";
				fout.close();
				fout.clear();
			};
		...
		else
		{
			...
			if(coupled_caspar_simulation){
				...
				fout << scientific << checkNanInfinity(myGapResult.time)											//1
					<< "\t" << checkNanInfinity(myGapResult.phi)													//2
					<< "\t" << checkNanInfinity(sum(myNewtonIteration.myPistonGap.forcespistongap.FTKy)) << "\n";		//3
				fout.close();
				fout.clear();
			};
		}
		...
		if( xold==0.0 || xold > (revcounter*revtime-tol) )
		{
			...
			if(myinput.data.options_piston.general.HeatTransfer)
			{
				...
				{
					fout << scientific << myNewtonIteration.myPistonGap.qbi_cylinder[0](i) << "\n";
				}
				fout << "%" << "\n";
				fout.close();
				fout.clear();
	...
	if( xold==0.0 || x>(xd2D+tol) ) 
	{
		...
		*/
		...
		fout << "VECTORS Gap_Height_[um] double"<<"\n";
		for(int i=0;i<Ncells;i++)
		{
			for(int j=0;j<Mcells;j++)
			{
		...
		fout << "\n";
		fout << "VECTORS Pressure_Deformation_[um] double" << "\n";
		for(int i = 0; i < Ncells ; i++)
		{
			for(int j = 0; j < Mcells ; j++)
			{
		...
		fout << "\n";
		fout << "VECTORS Thermal_Deformation_[um] double" << "\n";
		for(int i = 0; i < Ncells ; i++)
		{
			for(int j = 0; j < Mcells ; j++)
			{
