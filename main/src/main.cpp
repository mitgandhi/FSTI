	...
	if(argc > 1){
		...
		for(int a = 1;a<argc ;a++){
			...
			if(!strcmp(argv[a],"-q"))
				...
				cout << "     -f #     Viscosity in contact areas calculated with additional # fraction of contact stress." << endl;
				exit(1);
			}
		}
	}
...
void StartGapModule(void)
{
	...
	Log << "\n";
	Log << "SHS" << "\n";
	Log << "Smart Hydraulic Solutions" << "\n";
	Log << "* Version: " << PROJECT_VERSION << setw(50 - strlen(PROJECT_VERSION)) << "*" << endl;
	Log << "\n";
