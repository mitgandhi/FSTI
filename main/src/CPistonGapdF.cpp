...
void CPistonGap::PistonCalcExternalForces_inclined_piston(void){
		//Variable declaration
	double mK,lSK,beta,betamax,phi,omega,pDC,pCase,
		zRK,FDK,FaKz,FwK_inclined,FTKy,FTG,FTGx,FTGy,FAK_inclined,FSK,FSKx,FSKy,FKx,FKy,MKx,MKy,gamma,zeta, Fwkz, Fwky, FAKz, FAKy;
	...
	//Centrifugal componets fro transforming inclined piston frame to global frame
	Fwkz = FwK_inclined * sin(zeta);
	Fwky = FwK_inclined * cos(zeta);
	
	forcespistongap.FaK = FaKz;
	forcespistongap.FDK = FDK;
	forcespistongap.Fwk = FwK_inclined;
	
	/*if(myinput.data.options_piston.general.friction_less)
	{
	...
	// Fromm inclined plane frame to global frame
    ...
    forcespistongap.FKx = - (FSKy + FAKy) * sin(phi) + FSKx * cos(phi) + FTGx;
    forcespistongap.FKy = (FSKy + FAKy) * cos(phi) + FSKx * sin(phi) + Fwky + FTGy;
    ...
    // External moment components (x and y directions)
    forcespistongap.MKx = -zRK * ((FSKy + FAKy) * cos(phi) + FSKx * sin(phi)) - (zRK - lSK) * Fwky;
    ...
    forcespistongap.FSKx = FSKx;
    forcespistongap.FSKy = FSKy;
    forcespistongap.FAKz = FAKz;
    forcespistongap.FAKy = FAKy;
	forcespistongap.Fwky = Fwky;
	forcespistongap.Fwkz = Fwkz;
	forcespistongap.FAK_inclined=FAK_inclined;
...
void CPistonGap::PistonCalcExternalForces(void)
{
	//Variable declaration
	double mK,lSK,beta,betamax,phi,omega,pDC,pCase,
		zRK,FDK,FaKz,FwK,FTKy,FTG,FTGx,FTGy,FAK,FSK,FSKx,FSKy,FKx,FKy,MKx,MKy,gamma,FAKz;
	...
	if(myinput.data.lubrication_module.wobblepump)
	{
		FwK = 0.0;
	}
	else
